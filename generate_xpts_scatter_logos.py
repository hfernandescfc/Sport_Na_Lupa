"""
Versão de teste: Scatter xPts com LOGOS dos times em vez de círculos coloridos.

Renderiza card2_scatter_logos.html via Selenium + Edge headless → PNG
Saída: pending_posts/{date}_xpts-serie-b/02_xpts_scatter_logos.png
"""
...
# Nomes curtos para os rótulos do scatter
...
def _load_logos_as_base64() -> dict[str, str]:
    """Load all team logos from cache as base64 data URIs."""
    logos = {}
...
def generate_scatter_card(df: pd.DataFrame, logos: dict[str, str]) -> None:
    if "sos" not in df.columns:
        print(
            "\n⚠  Card requer a coluna 'sos' — rode primeiro:\n"
    ...
    tmp_html = OUT_DIR / "_scatter_logos_tmp.html"
    ...
    out_path = OUT_DIR / "02_xpts_scatter_logos.png"
    try:
        driver.get(tmp_html.as_uri())
        driver.set_window_size(1200, 1200)
        time.sleep(2.0)  # aguarda JS + imagens renderizarem
...
if __name__ == "__main__":
    ...
    print("\nGerando Card 02 (teste com logos)...")
    generate_scatter_card(df, logos)
